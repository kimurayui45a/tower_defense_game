...
public class PlacementCharaSelectPopUp : MonoBehaviour
{
    ...
    [SerializeField]
    private TMP_Text txtPickupCharaName;

    [SerializeField]
    private TMP_Text txtPickupCharaAttackPower;

    [SerializeField]
    private TMP_Text txtPickupCharaAttackRangeType;

    [SerializeField]
    private TMP_Text txtPickupCharaCost;

    [SerializeField]
    private TMP_Text txtPickupCharaMAxAttackCount;
    ...
    public void SetSelectCharaDetail(CharaData charaData)
    {
        if (charaData == null)
        {
            Debug.LogError("[SetSelectCharaDetail] charaData is NULL", this);
            return;
        }

        Debug.Log(
            $"[SetSelectCharaDetail] " +
            $"name:{charaData.charaName}, no:{charaData.charaNo}, cost:{charaData.cost}, " +
            $"atk:{charaData.attackPower}, range:{charaData.attackRange}, " +
            $"interval:{charaData.intervalAttackTime:0.00}s, max:{charaData.maxAttackCount}, " +
            $"sprite:{(charaData.charaSprite != null ? charaData.charaSprite.name : "null")}",
            this
        );
